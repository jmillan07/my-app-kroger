...
export function Administrator()  {
    ...
    const newlletter = useFormik({
        initialValues : {cedula:'', nombres:'', apellidos:'', correo:'',usuario:'',clave:'',key:1,vacunado:'nodefinido'},
        onSubmit,
        validate
    })
    ...
    function onSubmit(values){
        ...
        values.usuario = values.nombres.slice(0,1)+ values.apellidos.split(" ",1).join()
    ...
    function validate(values){
        ...
        for(let i =0; i<datoArray.length;i++){
            const dataObject =JSON.parse(datoArray[i])
            if (
                values.cedula === dataObject.cedula 
              ) {
    ...
    return (
        ...
        <form className="container" onSubmit={newlletter.handleSubmit}>
            ...
            <div style={{disply:"flex",width:500}}>
            <label>Nombre</label>
                <input type="text" id="nombres" name="nombres" value={newlletter.values.nombres} onChange={newlletter.handleChange}/>
                {newlletter.errors.nombres}
            </div>
            <div style={{disply:"flex",width:500}}>

            <label>Apellido</label>
                <input type="text" id="apellidos" name="apellidos" value={newlletter.values.apellidos} onChange={newlletter.handleChange}/>
                {newlletter.errors.apellidos}
            </div>
            <div style={{disply:"flex",width:500}}>
                <label>Correo</label>
                <input type="email" id="correo" name="correo" value={newlletter.values.correo} onChange={newlletter.handleChange}/>
                {newlletter.errors.correo}
            </div>
            <div style={{disply:"flex",width:500}}>
                <button type="submit">Submit</button>
                <button type="reset" onClick={newlletter.resetForm} >Reset</button>
            </div>
        </form>

        {newlletter.values.usuario ?
        <h3>El usuario que acabas de Genera es: {newlletter.values.usuario+"        "} 
         La clave es : {newlletter.values.clave}
        </h3>
        : null
        }

        <AdministratorDetail data={data} setData={setData} actualiza={actualiza} setActualiza={setActualiza}/>
        </>
    )
}